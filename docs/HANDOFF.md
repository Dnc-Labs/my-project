...
- ✅ **4.1 Shopping Cart — CODE XONG** (test E2E để sau) — `docs/4.1-shopping-cart.md`. Entity (owning/inverse side) + DTO + Repository (`findExisting` xử lý variant NULL, `findByUserIdWithItems` `@EntityGraph` chống N+1) + Mapper (`@AfterMapping` tính live price/subtotal/total) + Service (get-or-create, merge cộng dồn, check stock theo tổng mới, ownership 404, ACTIVE check) + Controller (5 endpoint, user từ `@AuthenticationPrincipal`). Postman group "7. Cart" (15 request) đã thêm — **chưa chạy test**. Docs có 2 deep-dive lớn: **`reduce` Stream API** + **N+1 query** (verbatim Q&A).
